...
std::vector<Texture> Model::LoadMaterialTextures(aiMaterial* mat, aiTextureType type, std::string typeName, const aiScene* scene) {
    ...
    for (unsigned int i = 0; i < mat->GetTextureCount(type); i++) {
        ...
        // (2) Mixamo FBX: 경로가 파일명인데 파일은 없고, 임베디드만 존재할 때  路径是文件名，但没有文件，只有嵌入式存在时
        // LoadMaterialTextures 함수에서 텍스처 매핑 수정  修改函数中的纹理映射
        if (!loaded && scene->mNumTextures > 0) {
            // === 기존 매핑이 잘못되었을 수 있으니 다른 순서로 시도  现有映射可能无效，请按不同的顺序尝试===
            int idx = -1;

            // 첫 번째 시도: 일반적인 순서 第一次尝试：一般顺序
            if (typeName == "texture_diffuse") idx = 1;      // 颜色纹理为1号
            else if (typeName == "texture_specular") idx = 0; // 灰度为0
        ...
        // (3) 외부 파일이 실제로 있으면 시도 (거의 필요 없음) 如果确实存在外部文件，则尝试（几乎不需要）
        if (!loaded) {
        ...
        if (!loaded) {
            std::cout << "[경고] 텍스처를 찾지 못함  未找到“警告”纹理: " << texPath << std::endl;
        }
    }
    return textures;
}
